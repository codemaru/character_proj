...
public class zombie_nav_Start : MonoBehaviour 
{
	...
	private NavMeshAgent zAgent;
	private float navSpeed = 1.0f;

	private bool playerDamaged = false;

	private float alertTimeOut = 5.0f; // 플레이어를 기억하는 시간
	private float alertTimer; // 추적 소요 시간(현재 지난 시간)
	...
	void Update () 
	{
		...
		if(alerted)
		{
			...
			if(attacking)
			{
				...
				if (attackTimer > 0.7f && playerDistance <= attackRange && !playerDamaged)
				{
					DamagePlayer();
				}
		...
		else
		{
			if(ptDistance <= changeDistance)
			{
				ChangePt();
				turning = true;
				zAgent.isStopped = true; // zAgent.Stop()
			}
		}
		if(turning)
		{
